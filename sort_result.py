...
avgTime = 0
avgRec = 0
for queryId, totalTime in sorted(totalTimeFile1.items(), key=lambda kv: (kv[1], kv[0])):
...
avgTime = 0
avgRec = 0
for queryId, totalTime in sorted(totalTimeFile2.items(), key=lambda kv: (kv[1], kv[0])):
    if numAdded >= minNumSolved:
        break
